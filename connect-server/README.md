...
reachable at `connect.hicalsoft.com`) instead of depending on third-party
TURN/signaling services, which is what fixes phone ↔ laptop calls getting stuck "searching"
...
- DNS: an `A`/`AAAA` record for `connect.hicalsoft.com` pointing at the Pi's
  public IP (this is a separate domain from `hicalsoft.com`'s GitHub Pages
  site — only this one subdomain needs to point at the Pi)
- nginx running on the Pi with certbot for TLS (a fresh install is fine —
  this doesn't need to share config with anything else already on the Pi)
...
```sh
sudo cp deploy/nginx-connect.conf /etc/nginx/sites-available/connect.hicalsoft.com
sudo ln -s /etc/nginx/sites-available/connect.hicalsoft.com /etc/nginx/sites-enabled/
sudo certbot --nginx -d connect.hicalsoft.com
...
```sh
curl https://connect.hicalsoft.com/healthz
curl https://connect.hicalsoft.com/turn-credentials
```
...
`wss://connect.hicalsoft.com/ws` and
`https://connect.hicalsoft.com/turn-credentials` — once this is deployed and
...
| `TURN_REALM`             | `connect.hicalsoft.com`    | TURN realm string |
